...
export default function Login() {
  ...
  return (
    <ResponsiveScreen>
      <GlassCard>
        <Text style={styles.h1}>{role === "teacher" ? "Teacher Login" : "Caregiver Login"}</Text>
        ...
        <Pressable style={styles.primaryBtn} onPress={submit} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryTxt}>Login</Text>}
        </Pressable>

        <Pressable onPress={() => router.push(`/register?role=${role}`)} style={styles.linkBtn}>
