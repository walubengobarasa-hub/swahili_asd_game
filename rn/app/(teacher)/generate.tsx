...
export default function TeacherGenerate() {
  ...
  const loadTopics = async () => {
    ...
    try {
      let rows;
      try {
        rows = await getJSON("/topics");
      } catch (err: any) {
        // fallback for older servers
        rows = await getJSON("/teacher/topics");
      }
  ...
  const generate = async () => {
    ...
    try {
      ...
      setErr(e?.message || "Imeshindwa kutengeneza kazi.");
    } finally {
      setBusy(false);
    }
  };
          ...
          <GlassCard>
            ...
            <View style={styles.rowWrap}>
              {loadingTopics ? (
                ...
                <Text style={styles.muted}>Hakuna mada (inatumia "{topic}").</Text>
              )}
            </View>
